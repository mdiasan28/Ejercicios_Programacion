...
public class Pelota extends Actor implements MouseListener, KeyListener {
	...
	//Contadores para las salidas de la pelota 
	...
	public void act() {
		if (trayectoria ==null) {
			...
			if (trayectoria ==null && milisAhora - milisEnInicio >5000 || contadorpelotaclick==1 || contadorpelotatiempo==1 || contadorpelotaespacio==1){
		...
		//Cuando la pelota salga por debajo la reseteamos
	...
	public void resetearPelota() {
		distanciasSiguienteFrame =5;
		this.x= Arkanoid.getInstance().getPlayer().getX()+18;
		this.y= Arkanoid.getInstance().getPlayer().getY()-15;
		contadorpelotaclick=0;
		contadorpelotaespacio=0;
		trayectoria =null;
		this.milisEnInicio = new Date().getTime();
		contadorvida--;
		if(contadorvida ==0) {
